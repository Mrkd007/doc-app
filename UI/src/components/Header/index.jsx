...
const HeaderComp = () => {
  ...
  return (
    <div className='doc-app__header'>
      <Header
        className='doc-app__header-comp'
      >
        ...
        />
        {user ? 
          <Dropdown.Button arrow className="doc-app__header-options" menu={menuProps} onClick={handleButtonClick}>
            {user.userName}
          </Dropdown.Button>
          :
          <Button type='primary' size='large' onClick={() => {navigate('/login')}}>Log In</Button>
        }
      </Header>
    </div>
  )
